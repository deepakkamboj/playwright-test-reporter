...
export class HistoryUtils {
   ...
   */
  static getLastRunInfo(outputDir: string = './test-results'): { status: string; failedTests: string[] } | null {
    try {
   ...
   */
  static compareWithPreviousRun(currentFailures: string[], outputDir: string = './test-results'): {
    newlyFailing: string[];
    fixed: string[];
  } {
    const lastRunInfo = this.getLastRunInfo(outputDir);
   ...
   */
  static getTestFailureHistory(outputDir: string = './test-results'): any[] {
    try {
